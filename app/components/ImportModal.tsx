    ...
    try {
      for (const task of data.tasks) {
        if (!task.id || !task.text || typeof task.completed !== "boolean") {
          return {
            success: false,
            message: t("import.errors.invalidTaskStructure"),
          };
        }
      }
      ...
      for (const category of data.categories) {
        if (!category.id || !category.name || !category.color) {
          return {
            success: false,
            message: t("import.errors.invalidCategoryStructure"),
          };
        }
      }
      ...
      if (generateNewIds) {
        ...
        updatedTasks = data.tasks.map((task) => {
          const newCategoryId =
            task.categoryId && idMap.has(task.categoryId)
              ? idMap.get(task.categoryId)
              : task.categoryId;
      ...
      const actionText = generateNewIds
        ? t("import.actionImported")
        : t("import.actionImportedUpdated");
      return {
        success: true,
      ...
      return {
        ...
        message: t("import.errors.importError", {
          error:
            error instanceof Error ? error.message : t("import.unknownError"),
        }),
      };
    }
  };
    ...
    try {
      ...
      if (result.success && result.data) {
        onImport(
          ...
          result.data.shouldOverride || false,
        );
      }
    } catch (error) {
      setResult({
              ...
              <div className="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                ...
                <ul className="text-xs text-amber-700 dark:text-amber-400 space-y-1">
                  <li>
                    •{" "}
                    {generateNewIds
                      ? t("import.importItem1a")
                      : t("import.importItem1b")}
                  </li>
                  <li>
                    •{" "}
                    {generateNewIds
                      ? t("import.importItem2a")
                      : t("import.importItem2b")}
                  </li>
                  <li>
                    •{" "}
                    {generateNewIds
                      ? t("import.importItem3a")
                      : t("import.importItem3b")}
                  </li>
                </ul>
              </div>
            </>
